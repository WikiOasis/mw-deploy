...
7. **Single sign-on**, optionally (on `/settings/authentication`, behind
   ...
   working either way, and you can switch the password form off once SSO is
   ...
   in is SSO are not asked to enrol TOTP here — nor is any account linked to it,
   so switch the password form off if you are relying on the provider for the
   second factor. The console also links an existing
